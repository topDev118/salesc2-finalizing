...
if($action == 'sendSticker') {
	...
	foreach ($categories as $key=>$value) {
		...
		$stickerstyle .= '.'.$value.'{ background: url("'.STATIC_CDN_URL.'plugins/stickers/images/'.$value.'/'.$value.'_1.png") no-repeat center}';
		...
		foreach ($images as $key=>$val) {
			if($val != '.' && $val != '..'){
				...
				$stickerstyle .= '.'.$class[0].'{ background-image: url("'.STATIC_CDN_URL.'plugins/stickers/images/'.$value.'/'.$val.'")}';
			}
		}
